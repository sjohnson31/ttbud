...
const getPlaywrightEnv = (basicEnv = 'node') => {
    ...
    return class PlaywrightEnvironment extends RootEnv {
        ...
        _getSeparateEnvBrowserConfig(isDebug, config) {
            ...
            let resultBrowserConfig = utils_1.deepMerge(defaultBrowserConfig, config);
            if (isDebug) {
                if (debugOptions) {
        ...
        async setup() {
            ...
            if (!skipInitialization) {
                ...
                if (collectCoverage) {
                    await this.global.context.exposeFunction('reportCodeCoverage', coverage_1.saveCoverageToFile);
                    await this.global.context.addInitScript(() => window.addEventListener('beforeunload', () => {
                        // @ts-ignore
